"""
==============================================================================
[PolyGlotFake 학습] 감정 흐름 기반 딥페이크 탐지 (영상)
HSEmotion + GRU + Attention - Trained on PolyGlotFake

[실행 위치]
~/hsh/AIApplication/reverse_zero_shot/
...
[출력 파일 (이 폴더 내)]
- emotion_flow_lite_pgf_best.pth
- emotion_lite_logs_pgf/history.csv
==============================================================================
"""
...
# 상위 폴더에 있는 emotion_deepfake_detector_lite.py 등을 import 가능하게
PARENT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
if PARENT_DIR not in sys.path:
    sys.path.insert(0, PARENT_DIR)

# 현재 폴더도 명시적으로 추가 (polyglotfake_data.py)
...
class PolyGlotFakeVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # video_path는 절대 경로 (polyglotfake_data.py에서 절대 경로로 생성됨)
...
def main():
    CFG = dict(
        # 출력 경로 (모두 현재 폴더 기준)
        CKPT_PATH       = "emotion_flow_lite_pgf_best.pth",
        LOG_DIR         = "emotion_lite_logs_pgf",

        # 학습 설정 (FakeAVCeleb과 동일)
        NUM_FRAMES      = 16,
        BATCH_SIZE      = 8,
        EPOCHS          = 15,python train_artifact_pgf.py --model both
        LR              = 1e-3,
        WEIGHT_DECAY    = 1e-4,
        GRU_HIDDEN      = 64,
        DROPOUT         = 0.3,
    ...
    train_loader = DataLoader(
        PolyGlotFakeVideoDataset(train_df, CFG['NUM_FRAMES']),
        batch_size=CFG['BATCH_SIZE'], shuffle=True,
        num_workers=CFG['NUM_WORKERS'], pin_memory=True
    )
    val_loader = DataLoader(
        PolyGlotFakeVideoDataset(val_df, CFG['NUM_FRAMES']),
    ...
    # ── 학습 ─────────────────────────────────────────────────
    print("\n" + "="*60)
    print("🚀 PolyGlotFake 학습 시작 (HSEmotion+GRU+Attention)")
    print("="*60)
