"""
==============================================================================
[PolyGlotFake 학습] MAE-DFER 백본 + GRU + Attention (Corrected Version)
...
class PolyGlotFakeVideoDataset(Dataset):
    def __init__(self, df, num_frames=16, mode='train'):
        ...
        self.mode = mode  # [수정] Transform 객체 대신 mode 문자열만 저장
...
def main():
    ...
    # ── 손실 함수 (pos_weight 안정화) ────────────────
    # PGF는 Real 766, Fake 2000 구조이므로, 비율 역수를 가중치로 사용
    num_neg = (train_df['video_label'] == 0).sum()
    num_pos = (train_df['video_label'] == 1).sum()
    weight_val = num_neg / num_pos if num_pos > 0 else 1.0
    pos_weight_val = torch.tensor([weight_val], dtype=torch.float32).to(device)
