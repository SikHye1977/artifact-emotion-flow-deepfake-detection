...
사용법:
  python3 train_nlp.py avdf1m   → nlp_unified_avdf1m_best.pth
  python3 train_nlp.py pgf      → nlp_unified_pgf_best.pth
"""
...
if mode == "avdf1m":
    CSV = os.path.join(RESULTS_DIR, "avdf1m_dataset.csv")
    SAVE = NLP_AVDF1M_CKPT
elif mode == "pgf":
    CSV = os.path.join(RESULTS_DIR, "pgf_dataset.csv")
    SAVE = NLP_PGF_CKPT
else:
    print("사용법: python3 train_nlp.py [avdf1m|pgf]"); sys.exit(1)
...
class TokenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prob = json.loads(r["token_probs"])[:ML]
        dur  = json.loads(r["token_durs"])[:ML]
        ...
        return {
            ...
            "token_probs":    torch.tensor(prob+[0.0]*pad, dtype=torch.float),
            "token_durs":     torch.tensor(dur+[0.0]*pad, dtype=torch.float),
...
def evaluate(model, loader):
    ...
    for b in loader:
        logits = model(b["input_ids"].to(DEVICE),
                       ...
                       b["token_probs"].to(DEVICE),
                       b["token_durs"].to(DEVICE))
        probs = torch.sigmoid(logits)
        vmask = (b["token_labels"].to(DEVICE) != -1).float()
        pv = probs * vmask
        k = min(NLP_CFG["topk"], pv.shape[1])
        topk_p,_ = torch.topk(pv, k, dim=1)
        topk_scores.extend(topk_p.mean(dim=1).cpu().numpy())
        labels.extend(b["clip_label"].numpy())
    labels = np.array(labels); scores = np.array(topk_scores)
...
model = UnifiedNLPModel(NLP_CFG["model_name"]).to(DEVICE)
...
for epoch in range(1, NLP_CFG["epochs"]+1):
    model.train(); tl = 0.0
    pbar = tqdm(train_loader, desc=f"Epoch {epoch}/{NLP_CFG['epochs']} [{mode}]")
    for b in pbar:
        optimizer.zero_grad()
        logits = model(b["input_ids"].to(DEVICE),
                       b["attention_mask"].to(DEVICE),
    ...
    if vm["auc"] > best_auc:
        ...
        torch.save({"epoch":epoch,"state_dict":model.state_dict(),
                    "val_auc":best_auc,"cfg":NLP_CFG}, SAVE)
        print(f"  ✅ Best 저장 (AUC={best_auc*100:.2f}%)")

with open(os.path.join(RESULTS_DIR,f"nlp_{mode}_train_log.json"),"w") as f:
    json.dump(log, f, indent=2)
print(f"\n✅ [{mode}] 학습 완료 | Best AUC: {best_auc*100:.2f}%")
print(f"   저장: {SAVE}")
