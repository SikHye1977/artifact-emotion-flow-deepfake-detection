"""
PGF audio technique attribution training.
...
python train_pgf_audio_tech.py \
  --model-type score \
  --feature-path features/pgf_balanced_100ptts.pt \
  --epochs 100
...
class PGFAudioTechDataset(Dataset):
    def __init__(
        self,
        feature_path: str,
        model_type: str,
        ...
        # audio_tech_label이 유효한 샘플만 사용
        self.features = [
            x for x in self.features
            if int(x.get("audio_tech_label", -1)) >= 0
        ]

        if len(self.features) == 0:
            raise RuntimeError("유효한 audio_tech_label을 가진 샘플이 없습니다.")

        self.audio_tech_map = data.get("pgf_audio_tech_map", {})
        ...
        print("[INFO] pgf audio tech map:", self.audio_tech_map)
    ...
    def _print_distribution(self):
        ...
        print("[INFO] audio tech distribution:")
        print(Counter([int(x["audio_tech_label"]) for x in self.features]))

        print("[INFO] audio model distribution:")
        print(Counter([x.get("audio_model", "") for x in self.features]))

        print("[INFO] split distribution:")
        print(Counter([x.get("split", "") for x in self.features]))
    ...
    def __getitem__(self, idx):
        ...
        y = torch.tensor(int(item["audio_tech_label"]), dtype=torch.long)

        meta = {
            "sample_id": item.get("sample_id", ""),
            "video_path": item.get("video_path", ""),
            "audio_model": item.get("audio_model", ""),
            "pgf_key": item.get("pgf_key", ""),
...
def stratified_split_by_audio_tech(
    dataset,
    train_ratio=0.7,
    val_ratio=0.15,
    seed=42,
    ...
    for i, item in enumerate(dataset.features):
        label = int(item["audio_tech_label"])
    ...
    def print_dist(name, indices):
        labels = [int(dataset.features[i]["audio_tech_label"]) for i in indices]
        print(f"[INFO] {name} audio_tech distribution:", Counter(labels))
...
class ScoreAudioTechMLP(nn.Module):
    def __init__(
        self,
        in_dim=8,
        hidden_dim=64,
...
class EmbeddingAudioTechMLP(nn.Module):
    def __init__(
        self,
        in_dim,
        hidden_dim=512,
...
class AATAudioTechTransformer(nn.Module):
    def __init__(
        self,
        video_token_dim,
        audio_token_dim,
...
def class_weights_from_dataset(dataset, train_idx, device, num_classes=6):
    labels = [int(dataset.features[i]["audio_tech_label"]) for i in train_idx]
...
def print_report(metrics, title):
    ...
    print(confusion_matrix(metrics["true"], metrics["pred"], labels=[0, 1, 2, 3, 4, 5]))
...
def main():
    ...
    if args.save_path is None:
        args.save_path = f"runs_pgf_audio_tech/best_{args.model_type}.pt"

    dataset = PGFAudioTechDataset(
    ...
    train_idx, val_idx, test_idx = stratified_split_by_audio_tech(
        dataset,
        train_ratio=0.7,
        val_ratio=0.15,
        seed=args.seed,
    ...
    class_weights = class_weights_from_dataset(dataset, train_idx, device, num_classes=6)
