"""
Step 2. Score-only MLP baseline for Attribution
...
class ScoreFeatureDataset(Dataset):
    def __init__(self, feature_path: str):
        ...
        # 직접 만든 feature file이므로 weights_only=False 사용
...
def stratified_split_by_label(dataset, label_name="fav_tech_label", train_ratio=0.7, val_ratio=0.15, seed=42):
    """
    fav_tech_label 기준 stratified split.
    20개/class 같은 소량 데이터에서 class 분포를 유지하기 위해 사용.
    """
...
class ScoreMLP(nn.Module):
    def __init__(
        ...
        self.modality_head = nn.Linear(hidden_dim, num_modality)
        self.tech_head = nn.Linear(hidden_dim, num_tech)
...
def evaluate(model, loader, device, split_name="val"):
    ...
    metrics = {
        ...
        "binary_macro_f1": f1_score(all_binary_true, all_binary_pred, average="macro", zero_division=0),

        "modality_acc": accuracy_score(all_mod_true, all_mod_pred),
        "modality_macro_f1": f1_score(all_mod_true, all_mod_pred, average="macro", zero_division=0),

        "tech_acc": accuracy_score(all_tech_true, all_tech_pred),
        "tech_macro_f1": f1_score(all_tech_true, all_tech_pred, average="macro", zero_division=0),
...
def main():
    ...
    parser.add_argument(
        "--save-path",
        type=str,
        default="runs_score_mlp/best_score_mlp.pt",
    )
